...
rpu_config.forward.out_noise = 0.03  # Some output noise.
...
def train(config=None):
    ...
    with wandb.init(config=config):
        ...
        subtask = "0" #str(int(np.log2(1/config.inp_res))) + "_" + str(int(np.log2(1/config.out_res))) + "_" + str(config.w_noise) + "_" + str(config.out_noise)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="RESNET18-GAN-w_noise1")
